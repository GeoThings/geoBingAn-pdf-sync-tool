...
### GET /api/groups/{group_id}/construction-reports/

取得群組下所有建案監測報告（分頁）。用於報告生成時比對建案與系統報告的對應關係。

**分頁參數**: `?page=1`（每頁 200 筆，自動遍歷所有頁面）

### GET /api/groups/{group_id}/construction-projects/

取得群組下所有建案專案清單。用於 `match_permits.py` 交叉比對建案名稱。

### GET /api/groups/{group_id}/construction-alerts/

取得群組下所有即時警戒值資料。用於 `match_permits.py` 取得建案名稱和警戒狀態（取代靜態 `alert_data.csv`）。

**回傳欄位（使用中）**:

| 欄位 | 說明 |
|------|------|
| `report_category_name` | 建案名稱（AI 解析） |
| `report_category` | 建照號碼 |
| `stage` | 施工階段 |
| `num_sensor` | 監測點數量 |
| `alerts` | 警戒值詳情（陣列） |

**待開發**: construction-reports API 目前不回傳 `report_category_name`，已建立 [ClickUp task #86ex8c82c](https://app.clickup.com/t/86ex8c82c) 請後端加入。
