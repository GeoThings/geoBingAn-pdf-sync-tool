...
11a. [上傳被擋下（exit 5：解析引擎探測）](#11a-上傳被擋下exit-5解析引擎探測)
...
## 11a. 上傳被擋下（exit 5：解析引擎探測）

### 症狀
`upload_pdfs` 印出「🛑 解析引擎異常，今日暫停上傳」後 exit 5（`EXIT_PARSER_HELD`；exit 4 另有其意＝重推查詢失敗）；同步失敗信寫「解析引擎異常，今日上傳已暫停」。
...
### 症狀
log 印出「⏹️ 已跨日（預留屬 YYYY-MM-DD），停止本批次；剩餘 N 份待下次執行於新的一天重新預留」。
