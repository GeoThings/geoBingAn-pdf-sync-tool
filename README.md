...
> ⚠️ 本表為靜態快照、僅在重大變化時手動更新；**即時數據以下方
> [線上追蹤報告](#-線上追蹤報告)為準（每日自動更新）**。

| 項目 | 數量 | 說明 |
|------|------|------|
| 建案資料夾 | 1,758 個 | Shared Drive 全量（完整翻頁掃描，#65/#68） |
| 建案 PDF inventory | 28,608 個 | 已對應建案資料夾的 PDF（state/pdf_inventory.json，每日更新） |
| 已上傳至究平安 | 776 份 | 30 天滾動窗 + catchup 補傳（#65 事故前後共補 546 份） |
| 建案名稱覆蓋 | 360/411 筆（88%） | API 優先 + 6 來源交叉比對 |
| 每日排程 | 10:00 | launchd 觸發：同步 → 上傳 → 名稱比對 → 報告 → GitHub |
| 執行時間 | ~25 分鐘 | 全量翻頁掃描後的正常水位 |
...
**功能特色：**
- ✅ 檔名日期智慧解析（民國年/西元年 9 種格式，含裸民國年前綴+MMDD）
...
python3 -m geobingan_sync.steps.upload_pdfs --catchup-days 62 --yes  # 大批次：仍受日上限裁切
...
- ✅ 抽出 `select_pdfs_to_upload()` 純函式（#64/#73）：候選過濾可單元測試；順修 production cutoff 帶時分秒錯殺 cutoff 當日報告的 bug
- ✅ parser 支援裸民國年前綴+MMDD（#75，裕光東湖 case）：全量 diff 0 regression、救回 70 筆（意外含 `112/`、`114.11/` 民國年資料夾前綴）
- ✅ catchup 180 天補傳 380/380（裕光東湖 5 份 + #65 截斷時代 1/30–4/29 窗口漏網）；累計補回 546 份
- 🧪 測試 175 → 193
...
- ✅ 間接連結解析：來源不是 Drive 資料夾時（Google Sites 頁面、短網址）自動再走一步找出背後的 Drive 資料夾；實測 73 個非 Drive 連結救回 30 個，可驗證母體 366 → 396 案。含 SSRF 防護（逐跳拒絕內網位址）與 14 天快取
- ✅ 檔名日期理智區間：解析結果若為**未來日**或早於 2000 年一律視為解析失敗（不上傳、不計入新鮮度、排序排最後）；西元年中文格式優先於民國年，避免 `2026年08月21日` 被讀成民國 26 年

### v4.2.0 (2026-04-15)
- ✅ 遞迴掃描子資料夾：18,931 → 26,820 個 PDF（+7,889 個來自 1,222 個子資料夾）
- ✅ API 報告模糊匹配：名稱滑動視窗匹配，對應成功 15,002 筆
- ✅ 監測警戒欄位重構：「異常紀錄」→「監測警戒」，顯示即時狀態 + 觸發日期
- ✅ 業界術語：⚠️ 警戒值 / 🔴 行動值（取代累計次數）
- ✅ 名稱優化：API 名稱自動取代通用/短名稱（監測報告、工地監測數據等 → API project 正式名稱）
- ✅ API project 匹配：580 個 construction-projects 去重 + 滑動視窗，116 筆建案匹配
- ✅ 括號名稱解析：支援 `(嘉鎷)115.01監測月報.pdf` 格式
- ✅ 通用名稱過濾：防止「監測」「監測報告」等誤配大量不相關報告
- ✅ 警戒值清除機制：每次比對清除舊的 live_alerts，避免殘留錯誤匹配
- ✅ 最近更新日期修正：取 max(Drive 時間, API 報告 created_at)
...
- ✅ 預設排序改為最近更新排前面
- ✅ 究心黑紅品牌主題（深黑 Header + 紅色點綴 + 等寬數字）
- ✅ SaaS 級表格 UX（排序箭頭、nowrap 防斷行、進度條 flexbox、空狀態提示）
- ✅ 觸控優化（擴大點擊熱區、搜尋 debounce 250ms）
- ✅ 無障礙（aria-pressed、手機不隱藏欄位）
- ✅ 用語白話化（等待同步、AI 處理中、異常紀錄 + 次數單位）
- ✅ Token 過期預警（LINE Notify，< 2 天警告 / 已過期中止）
- ✅ 上傳歷史持久化到 git（防 fresh clone 重複上傳）

### v3.8.0 (2026-04-13)
- ⚡ 報告 UX 改善（表格間距、badge 圖示、Segmented Control、Drive ↗ 圖示）

### v3.7.0 (2026-04-13)
- ✅ 上傳歷史持久化到 git（防止 fresh clone 重複上傳）
- ✅ load_state() 自動合併 git 追蹤的上傳記錄
- ✅ push 條件改為 git diff --cached（history 變更也會推送）

### v3.6.0 (2026-04-07)
- ✅ 報告可讀性改善：統計卡片、表格欄位、狀態 badge、filter 按鈕全部改為易懂中文
- ✅ 新增「📖 圖例說明」可收合區塊（狀態顏色意義 + 欄位說明 + 行動指引）
- ✅ 統計卡片加入 tooltip 說明
- ✅ 「需要關注」改為「需要處理」行動導向標題
