...
| 項目 | 狀態 | 數量 |
|---------|------|--------|
| 建案監測 | ✅ 已同步 | 292 個建案（新增 14 個 PDF） |
| Google Drive PDF | ✅ 掃描完成 | 11,636 個 |
| 農曆新年後上傳 | ✅ 完成 | 68 個 PDF 上傳至究平安 |
| 究平安報告對應 | ✅ 成功 | 18,901 筆 / 1,065 筆對應 |
| JWT 自動刷新 | ✅ 正常 | Token 過期自動更新 |
| 執行時間 | ✅ | ~11 分鐘（v3.4 並行優化） |

### 📋 線上追蹤報告

👉 **[建照監測追蹤報告](https://htmlpreview.github.io/?https://github.com/GeoThings/geoBingAn-pdf-sync-tool/blob/main/docs/index.html)** 👈

報告包含：需要處理儀表板、建案同步狀態、警戒值標記、圖例說明、搜尋篩選
...
### v3.6.0 (2026-04-07)
- ✅ 報告可讀性改善：統計卡片、欄位、badge、filter 按鈕全面中文化
- ✅ 新增「📖 圖例說明」收合區塊（狀態顏色 + 行動指引 + 欄位說明）
- ✅ 統計卡片加入 tooltip 說明（滑鼠懸停顯示解釋）
- ✅ 「需要關注」改為「⚠️ 需要處理」具體行動導向描述

### v3.5.0 (2026-04-02)
- ✅ 報告重新設計：「需要處理」儀表板（警戒值 + 過期報告）
